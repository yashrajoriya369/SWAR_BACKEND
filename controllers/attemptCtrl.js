...
const startAttempt = asyncHandler(async (req, res) => {
  const userId = req.user?.id;
  const quizId = req.params.quizId;
  const forceAttempt = req.query.force === "true"; // optional query param
  ...
  if (quiz.attemptType === "Single" && !forceAttempt) {
  ...
  // Create new attempt (always for multiple or forceAttempt)
  const attempt = await Attempt.create({
    quizId,
    userId,
    startedAt: new Date(),
